...
(struct bird_s (y v-y))
(struct pipe_s (gap-y gap-x))
(struct pipes_s (l))
(struct variables (gravity momentum scroll-speed))

(struct state_s (bird_s pipes_s variables score))

(define (get-gravity state)
  (variables-gravity (state_s-variables state)))
(define (get-momentum state)
  (variables-momentum (state_s-variables state)))
(define (get-scroll-speed state)
  (variables-scroll-speed (state_s-variables state)))
...
(define (get-initial-state)
   (state_s [bird_s bird-initial-y 0]
            [pipes_s (list (pipe_s (+ added-number (random random-threshold)) scene-width))]
            [variables initial-gravity initial-momentum initial-scroll-speed]
            0))
...
(define (next-state-bird bird gravity)
  (struct-copy bird_s bird
               [y (+ (get-bird-y bird) (get-bird-v-y bird))]
               [v-y (+ (bird_s-v-y bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird_s bird [v-y (* -1 momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy state_s current-state [bird_s (next-state-bird-onspace (get-bird current-state) (get-momentum current-state))])
      current-state))
...
(define (get-pipes state)
  (pipes_s-l (state_s-pipes_s state)))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (pipe) (struct-copy pipe_s pipe [gap-x (- (get-pipe-x pipe) scroll-speed)])) pipes))
...
(define (add-more-pipes pipes)
    (if (< (length pipes) no-pipes)
        (cons (pipe_s (+ added-number (random random-threshold)) (+ pipe-width pipe-gap (get-pipe-x (car pipes)))) pipes)
        pipes))
...
(define (check-pipe-collisions bird pipes)
   (let* ([bird_p1 (make-posn bird-x (get-bird-y bird))] [bird_p2 (make-posn (+ bird-x bird-width) (+ (posn-y bird_p1) bird-height))])  
     (not (empty? (filter (lambda (pipe) (match-let ([(pipe_s y x) pipe])
                                      (or (check-collision-rectangles bird_p1 bird_p2 (make-posn x 0) (make-posn (+ x pipe-width) y)) ; upper pipe
                                          (check-collision-rectangles bird_p1 bird_p2 (make-posn x (+ y pipe-self-gap)) (make-posn (+ x pipe-width) pipe-height))))) ; lower pipe
                          pipes)))))
...
(define (next-state state)
  (struct-copy state_s state
               [bird_s (next-state-bird (get-bird state) (get-gravity state))]
               [pipes_s (pipes_s (next-state-pipes (get-pipes state) (get-scroll-speed state)))]
               [score (+ 0.1 (get-score state))]))
...
(define (place-pipes pipes scene)
  (let ([half-pipe-width (quotient pipe-width 2)])
	(foldl (lambda (pipe acc) (let* ([x (get-pipe-x pipe)] [y (pipe_s-gap-y pipe)] [lower-pipe-height (- pipe-height y pipe-self-gap)])
                                    (place-images
                                     (list
                                      (rectangle pipe-width y "solid" "green") ; upper pipe
                                      (rectangle pipe-width lower-pipe-height "solid" "green")) ; lower pipe
                                     (list
                                      (make-posn (+ x half-pipe-width) (quotient y 2)) ; upper pipe
                                      (make-posn (+ x half-pipe-width) (- pipe-height (quotient lower-pipe-height 2)))) ; lower pipe
                                     acc)))
               scene pipes)))
